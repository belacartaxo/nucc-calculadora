...
const Calculadora = () => {
  ...
  const handleOperador = (operador) => {
    if (!n1 || isIgualPressionado) return; // Evita operadores sem número ou após "="
    if (op && n2) {
  ...
  const calcular = () => {
    ...
    setN1(resultadoFinal.toString());
    setN2("");
    setOp("");
    setIsIgualPressionado(true);
  };
      ...
      <div className="row">
        {["7", "8", "9", "*"].map((item) => (
          <div
            ...
            className="tecla"
            onClick={() =>
              operadores.includes(item) ? handleOperador(item) : handleNumero(item)
            }
          >
            {item}
          </div>
